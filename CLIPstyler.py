...
class CLIPstyler():
    def __init__(self, args):
        ...
        self.lambda_grob = CFG.lambda_grob
        self.lambda_content = CFG.lambda_content
        self.lambda_patch = CFG.lambda_patch
    ...
    def train(self):
        ...
        if self.wandb:
            ...
            wandb.watch(models=(self.style_net), log_freq=10)
        ...
        for step in range(CFG.step):
            ...
            self.target = self.style_net(self.content_img)
            ...
            if self.wandb:
                # losses["patch_loss_hist"] = 
                # wandb.log({"hist":wandb.Histogram(loss_temp.detach().cpu().numpy())})
                wandb.log(losses)
